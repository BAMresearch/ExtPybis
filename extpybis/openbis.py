...
class ExtOpenbis(Openbis):
    ...
    def get_metadata_import_template(
        ...
        meta_list = list(self.get_sample_type(sample_type).get_property_assignments().df["propertyType"])
        meta_dict = {idx: v for idx, v in enumerate(meta_list)}

        # Parse the dict into a dataframe
        meta_df = pd.DataFrame.from_dict(meta_dict, orient="index", columns=["Param"])

        # Get property descriptions
        props_list = list(self.get_sample_type(sample_type).get_property_assignments().df["propertyType"])
        ...
        for prop in props_list:
            ...
            prop_df = pd.DataFrame({prop: prop_series.values}, index=prop_series.index)
        ...
        for key in metadata.keys():
            ...
            if property_data_type == "BOOLEAN":
                if metadata[key] in ["True", "False"]:
                    ...
                    raise KeyError(
                        f'Invalid key "{key}" for dataType BOOLEAN. '
    ...
    def get_sample_type_properties(self, sample_type: Union[SampleType, str]) -> pd.DataFrame:
        ...
        props_list = list(sample_type.get_property_assignments().df["propertyType"])
    ...
    def set_parent_annotation(
        self, child_sample: str, parent_sample: str, comment: str, set_property: Optional[str] = None
    ):
        """
        ...
        response = requests.post(
            url=combine_urls(self.url, self.as_v3), json=request, verify=self.verify_certificates
        ).json()
        return response
    ...
    def generate_typechecker(self, sample_type: Union[SampleType, str]) -> ModelMetaclass:
        ...
        mandatory_props_dict = pd.Series(
            mandatory_props_df.mandatory.values, index=mandatory_props_df.propertyType
        ).to_dict()
        ...
        property_function_input = {
            key: (self._get_datatype_conversion(key, val), None if key not in mandatory_props else ...)
